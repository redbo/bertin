...
func (m ServerConfig) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
    ...
    switch request.Method {
        case "GET": ObjGetHandler(writer, request, vars, m)
        case "HEAD": ObjGetHandler(writer, request, vars, m)
        case "PUT": ObjPutHandler(writer, request, vars, m)
        case "DELETE": ObjDeleteHandler(writer, request, vars, m)
    }
}
